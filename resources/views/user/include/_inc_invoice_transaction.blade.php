<div class="wrapper" id="wrapper" style="max-width: 780px;margin: 20px auto;padding: 5px 10px;background-color: white;height: 75%;overflow: hidden;">
	...
	<div style="display: flex;justify-content: space-between;margin-bottom: 10px">
		...
		@if(Route::current()->getName() == 'ajax.admin.transaction.invoice')
			<div style="text-align: left;width: 50%">
			<p style="margin: 0">Người lập <b>{{get_data_user('admins','name')}}</b></p>
			<p style="margin: 0">Địa chỉ <b>{{get_data_user('admins','address')}}</b> </p>
				<p style="margin: 0">Điện thoại <b>{{get_data_user('admins','phone')}}</b></p>
			</div>
		@else
			<div style="text-align: left;width: 50%">
		...
		<div class="table table-striped">
			<table style="width: 100%">
				...
				@foreach($orders as $key => $order)
					@php
					$itemTotal = $order->od_price * $order->od_qty;
	...
	<br>
	<br>
	<p style="margin-bottom: 5px;text-align: right">Tổng tiền hàng:  <b>{{ number_format($totalMoney,0,',','.') }} đ</b></p>
	...
	@endphp
	<p style="margin-bottom: 5px;text-align: right">Giảm tối đa {{$couponDetail->cp_discount}} % (Code: {{$couponDetail->cp_code}}):  <b>-{{ number_format($totalDiscount,0,',','.') }} đ</b></p>
	@endif
	<p style="margin-bottom: 5px;text-align: right">Tổng tiền cần thanh toán:  <b>{{ number_format($transaction->tst_total_money,0,',','.') }} đ</b></p>
