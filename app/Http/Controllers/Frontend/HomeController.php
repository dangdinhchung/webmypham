    ...
    public function index()
    {
        ...
        $productsNew = Cache::remember('HOME.PRODUCT_NEW', 60 * 24 * 30, function () {
            return Product::where('pro_active', 1)
                ->orderByDesc('id')
                ->limit(4)
                ->select('id', 'pro_name', 'pro_slug', 'pro_sale', 'pro_avatar', 'pro_number', 'pro_price',
...
//        });
    ...
    public function getLoadProductByCategory(Request $request)
    {
        if ($request->ajax()) {
                ...
                return Category::with([
                    'products' => function ($q) {
                        $q->where('pro_active', 1)
                            ->select('id', 'pro_name', 'pro_slug', 'pro_category_id', 'pro_sale', 'pro_avatar',
                                'pro_number', 'pro_price', 'pro_review_total', 'pro_review_star')
                            ->limit(20)
                            ->orderByDesc('id')
                            ->get();
                    }
            ...
            $html = view('frontend.pages.home.include._inc_product_by_category_hot',
                compact('categoriesHot'))->render();
            return response()->json(['data' => $html]);
        }
    }
